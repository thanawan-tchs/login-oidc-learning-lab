...
export async function POST(request, { params }) {
  ...
  if (otp.expiresAt < Date.now()) {
    ...
    return NextResponse.redirect(absoluteUrl(`/interaction/${uid}/verify-otp?error=expired`), 303);
  }
  ...
  if (submitted !== otp.code) {
    ...
    if (otp.attempts >= MAX_ATTEMPTS) {
      delete interaction.otp;
      await interactions.set(uid, interaction);
      return NextResponse.redirect(absoluteUrl(`/interaction/${uid}/verify-otp?error=too_many_attempts`), 303);
    }
    await interactions.set(uid, interaction);
    return NextResponse.redirect(absoluteUrl(`/interaction/${uid}/verify-otp?error=wrong_code`), 303);
  }
